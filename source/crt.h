...
const char*							szprint(char* result, const char* result_maximum, const char* format, ...);
const char*							szprintv(char* result, const char* result_maximum, const char* format, const char* vl);
...
template<unsigned N> const char*	zprint(char(&result)[N], const char* format, ...) { return szprintv(result, result + N - 1, format, xva_start(format)); }
